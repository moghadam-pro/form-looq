...
final class Form_Manager {
	...
	public static function render_builder( \WP_Post $post ): void {
		...
		<input id="fmpf-submission-type" name="fmpf_submission_type" class="regular-text" value="<?php echo esc_attr( $type ); ?>" pattern="[a-z0-9_-]+">
		...
		<textarea id="fmpf-fields" name="fmpf_fields" class="large-text code" rows="16" spellcheck="false"><?php echo esc_textarea( (string) $fields ); ?></textarea>
	...
	public static function save( int $post_id, \WP_Post $post ): void {
		if ( ! isset( $_POST['fmpf_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['fmpf_nonce'] ) ), 'fmpf_save_form' ) || ! current_user_can( 'edit_post', $post_id ) || wp_is_post_revision( $post_id ) ) {
			return;
		}
		$fields = isset( $_POST['fmpf_fields'] ) ? sanitize_textarea_field( wp_unslash( $_POST['fmpf_fields'] ) ) : '';
		$type   = isset( $_POST['fmpf_submission_type'] ) ? sanitize_key( wp_unslash( $_POST['fmpf_submission_type'] ) ) : 'general';
	...
	public static function shortcode( array $atts ): string {
		...
		$atts = shortcode_atts(
			array(
				...
				'error'  => __( 'Please review the required fields and try again.', 'free-mpro-forms' ),
				'dir'    => 'auto',
				'select' => '',
				'yes'    => '',
			),
		...
		<div class="fmpf-form-wrap" dir="<?php echo esc_attr( $direction ); ?>">
			<?php if ( 'sent' === $status ) : ?><p class="fmpf-notice" role="status"><?php echo esc_html( (string) $atts['sent'] ); ?></p><?php endif; ?>
			<?php if ( 'error' === $status ) : ?><p class="fmpf-notice is-error" role="alert"><?php echo esc_html( (string) $atts['error'] ); ?></p><?php endif; ?>
			<form class="fmpf-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<label class="fmpf-honeypot" aria-hidden="true">Website<input name="website" tabindex="-1" autocomplete="off"></label>
	...
	public static function handle_submission(): void {
		...
		if ( ! $form_id || 'fmpf_form' !== get_post_type( $form_id ) || 'publish' !== get_post_status( $form_id ) || ! isset( $_POST['fmpf_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['fmpf_nonce'] ) ), 'fmpf_submit_' . $form_id ) || ! empty( $_POST['website'] ) ) {
		...
		if ( ! Submission_Manager::store( $type ?: 'general', $title, $data, $form_id ) ) {
			self::redirect( $referer, 'error' );
		}
	...
	private static function parse_fields( string $definition ): array {
		...
		foreach ( preg_split( '/\R/u', $definition ) ?: array() as $line ) {
			...
			$options = array_values( array_filter( array_map( 'sanitize_text_field', explode( ',', $parts[4] ) ) ) );
			if ( 'scale' === $type && ! $options ) {
				$options = array( '1', '2', '3', '4', '5' );
			}
			if ( ! in_array( $type, array( 'section', 'text', 'email', 'tel', 'number', 'textarea', 'select', 'radio', 'scale', 'checkbox' ), true ) || ( 'section' !== $type && ! $name ) || ! $label ) {
				continue;
			}
			$fields[] = array(
				'type'     => $type,
	...
	private static function render_field( array $field, string $select_label, string $yes_label ): void {
		...
		$class    = in_array( $field['type'], array( 'textarea', 'radio', 'scale', 'checkbox' ), true ) ? ' class="fmpf-field fmpf-field--wide"' : ' class="fmpf-field"';
		echo '<div' . $class . '><span class="fmpf-label">' . esc_html( $label ) . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		if ( 'textarea' === $field['type'] ) {
			...
			echo '<label class="fmpf-checkbox"><input type="checkbox" name="' . esc_attr( $field['name'] ) . '" value="1"' . $required . '><span>' . esc_html( $yes_label ) . '</span></label>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		} else {
		...
		if ( $field['help'] ) {
			echo '<small class="fmpf-help">' . esc_html( $field['help'] ) . '</small>';
		}
		echo '</div>';
	}
