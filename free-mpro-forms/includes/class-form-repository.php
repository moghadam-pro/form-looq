...
final class Form_Repository {
	...
	private const CACHE_GROUP = 'fmpf_forms';
	...
	public static function statuses(): array {
		return array(
			self::STATUS_ACTIVE   => __( 'Active', 'free-mpro-forms' ),
			self::STATUS_INACTIVE => __( 'Inactive', 'free-mpro-forms' ),
			self::STATUS_DRAFT    => __( 'Draft', 'free-mpro-forms' ),
		);
	}
	...
	public static function create( array $data ): int {
		 ...
		 */
		do_action( 'free_mpro_forms_form_created', $form_id );
	...
	public static function update( int $form_id, array $data ): bool {
		 ...
		 */
		do_action( 'free_mpro_forms_form_updated', $form_id );
	...
	public static function duplicate( int $form_id ): int {
		...
		return self::create(
			array(
				...
				'title'       => sprintf( __( '%s (copy)', 'free-mpro-forms' ), $form['title'] ),
	...
	public static function delete( int $form_id ): bool {
		 ...
		 */
		do_action( 'free_mpro_forms_form_deleted', $form_id );
	...
	public static function record_view( int $form_id ): void {
		...
		if ( $form_id <= 0 || ! apply_filters( 'free_mpro_forms_track_views', true, $form_id ) ) {
			return;
		}
	...
	private static function sanitize_title( string $title ): string {
		...
		$title = '' !== $title ? $title : __( 'Untitled form', 'free-mpro-forms' );
	...
	public static function sanitize_settings( array $settings ): array {
		...
		if ( '' === $clean['submit_label'] ) {
			$clean['submit_label'] = __( 'Submit', 'free-mpro-forms' );
		}

		if ( '' === $clean['success_message'] ) {
			$clean['success_message'] = __( 'Thank you. Your submission has been received.', 'free-mpro-forms' );
		}
