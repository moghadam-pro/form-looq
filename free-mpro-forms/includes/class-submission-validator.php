...
final class Submission_Validator {
	...
	public static function validate( array $fields, array $request ): array {
		...
		$allowed_keys = array_merge(
			$field_keys,
			array( 'action', 'form_id', 'fmpf_nonce', 'fmpf_started_at', 'website', '_wp_http_referer' )
		);
		$unexpected   = array_diff( array_keys( $request ), $allowed_keys );

		if ( $unexpected ) {
			$errors['_form'] = __( 'The submission contained unexpected fields. Please reload the page and try again.', 'free-mpro-forms' );
		}
		...
		switch ( $code ) {
			case 'required':
				return sprintf(
					...
					__( '%s is required.', 'free-mpro-forms' ),
					$field['label']
				);
			case 'email':
				return __( 'Enter a valid email address.', 'free-mpro-forms' );
			case 'number':
				return __( 'Enter a valid number.', 'free-mpro-forms' );
			case 'tel':
				return __( 'Enter a valid telephone number.', 'free-mpro-forms' );
			case 'choice':
				return __( 'Select one of the available options.', 'free-mpro-forms' );
			case 'length':
				return __( 'This value is longer than the allowed limit.', 'free-mpro-forms' );
			default:
				return __( 'Review this field and try again.', 'free-mpro-forms' );
		}
	}
