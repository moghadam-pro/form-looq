/**
 * Free MPRO Forms — shared admin behaviour.
 *
...
( function () {
	...
	var config = window.fmpfAdmin || {};

	function announce( message ) {
		var notice = document.createElement( 'div' );
		notice.className = 'notice notice-success is-dismissible fmpf-flash';
		notice.setAttribute( 'role', 'status' );
		notice.innerHTML = '<p></p>';
		notice.querySelector( 'p' ).textContent = message;

		var anchor = document.querySelector( '.fmpf-header' );
	...
	document.addEventListener( 'click', function ( event ) {
		var copyButton = event.target.closest( '[data-fmpf-copy]' );
		if ( copyButton ) {
			event.preventDefault();
			copyFrom( copyButton.getAttribute( 'data-fmpf-copy' ) );
			return;
		}

		var confirmLink = event.target.closest( '[data-fmpf-confirm]' );
		if ( confirmLink && ! window.confirm( confirmLink.getAttribute( 'data-fmpf-confirm' ) ) ) {
			event.preventDefault();
		}
	} );

	// Warn before navigating away from a builder form with unsaved edits.
	var builderForm = document.getElementById( 'fmpf-builder-form' );
