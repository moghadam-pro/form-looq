/**
 * Free MPRO Forms — drag-and-drop form builder.
 *
...
( function () {
	...
	var config = window.fmpfBuilder || {};
	var LABELS = config.labels || {};
	var TYPES = config.types || [];
	var MAX_FIELDS = 50;

	var canvas = document.getElementById( 'fmpf-canvas' );
	var palette = document.getElementById( 'fmpf-palette' );
	var input = document.getElementById( 'fmpf-fields-input' );
	var form = document.getElementById( 'fmpf-builder-form' );
	...
	function labelled( labelText, control, id ) {
		var wrap = el( 'p', 'fmpf-card__control' );
	...
	function renderCard( field, index ) {
		...
		var card = el( 'div', 'fmpf-card' );
		card.setAttribute( 'draggable', 'true' );
		card.setAttribute( 'data-index', String( index ) );

		var head = el( 'div', 'fmpf-card__head' );
		var handle = el( 'span', 'fmpf-card__handle dashicons dashicons-move' );
		handle.setAttribute( 'aria-hidden', 'true' );
		head.appendChild( handle );

		var icon = el( 'span', 'fmpf-card__icon dashicons dashicons-' + meta.icon );
		icon.setAttribute( 'aria-hidden', 'true' );
		head.appendChild( icon );

		head.appendChild( el( 'strong', 'fmpf-card__type', meta.label ) );
		head.appendChild( el( 'span', 'fmpf-card__summary', field.label || '' ) );

		var tools = el( 'span', 'fmpf-card__tools' );
		...
		var body = el( 'div', 'fmpf-card__body' );
		var prefix = 'fmpf-field-' + index + '-';

		body.appendChild( labelled( LABELS.label, textInput( field.label, function ( value ) {
			field.label = value;
			var summary = card.querySelector( '.fmpf-card__summary' );
			if ( summary ) {
		...
		if ( 'section' !== field.type ) {
			var flags = el( 'p', 'fmpf-card__flags' );
			...
			var widthLabel = el( 'label', 'fmpf-card__width' );
		...
		card.addEventListener( 'drop', function ( event ) {
			...
			var newType = event.dataTransfer.getData( 'fmpf/type' );
			if ( newType ) {
	...
	function toolButton( icon, label, onClick ) {
		...
		button.type = 'button';
		button.className = 'fmpf-card__tool';
		button.title = label;
	...
	function render() {
		...
		if ( ! fields.length ) {
			canvas.appendChild( el( 'p', 'fmpf-canvas__empty', LABELS.empty ) );
			sync();
			return;
		}
	...
	if ( palette ) {
		palette.addEventListener( 'click', function ( event ) {
			var button = event.target.closest( '.fmpf-palette__item' );
			if ( button ) {
				insert( makeField( button.getAttribute( 'data-fmpf-type' ) ) );
			}
		} );
		...
		palette.addEventListener( 'dragstart', function ( event ) {
			...
			event.dataTransfer.setData( 'fmpf/type', button.getAttribute( 'data-fmpf-type' ) );
			event.dataTransfer.setData( 'text/plain', button.getAttribute( 'data-fmpf-type' ) );
		} );
	}
