...
final class Frontend_Form {
	public static function init(): void {
		...
		// Pre-rename alias, so content saved as [free_mpro_form] keeps rendering.
		add_shortcode( 'free_mpro_form', array( self::class, 'shortcode' ) );
		add_action( 'wp_enqueue_scripts', array( self::class, 'register_assets' ) );
		add_action( 'admin_post_mpro_submit', array( self::class, 'handle_submission' ) );
		add_action( 'admin_post_nopriv_mpro_submit', array( self::class, 'handle_submission' ) );
	...
	public static function register_assets(): void {
		...
		wp_register_style(
			'mpro-forms',
			plugins_url( 'assets/css/forms.css', MPRO_FORMS_FILE ),
			array(),
			MPRO_FORMS_VERSION
		);
	}
	...
	public static function shortcode( array $atts ): string {
		$atts = shortcode_atts(
			array(
				...
				'button' => '',
				'sent'   => '',
				'error'  => __( 'Please review the highlighted fields and try again.', 'mpro-forms' ),
				'dir'    => 'auto',
				'select' => '',
				'yes'    => '',
			),
		...
		if ( ! $form ) {
			return Plugin::current_user_can() ? '<p>' . esc_html__( 'Select an active form.', 'mpro-forms' ) . '</p>' : '';
		}
		...
		wp_enqueue_style( 'mpro-forms' );
		wp_enqueue_script(
			'mpro-forms-frontend',
			plugins_url( 'assets/js/forms.js', MPRO_FORMS_FILE ),
			array(),
			MPRO_FORMS_VERSION,
			true
		);
		...
		$notice_id   = 'mpro-notice-' . $form_id;
		$form_id_attr = 'mpro-form-' . $form_id;

		$layout = 'two-column' === ( $settings['layout'] ?? '' ) ? ' mpro-layout-two-column' : '';

		ob_start();
		?>
		<div id="<?php echo esc_attr( $form_id_attr ); ?>" class="mpro-form-wrap<?php echo esc_attr( $layout ); ?>" dir="<?php echo esc_attr( $direction ); ?>" data-mpro-has-errors="<?php echo $has_error ? '1' : '0'; ?>">
			<?php if ( 'sent' === $status ) : ?>
				<p id="<?php echo esc_attr( $notice_id ); ?>" class="mpro-notice" role="status" tabindex="-1"><?php echo esc_html( (string) $atts['sent'] ); ?></p>
			<?php elseif ( $has_error ) : ?>
				<?php self::render_error_summary( $fields, $errors, (string) $atts['error'], $notice_id, $form_id ); ?>
			<?php endif; ?>

			<form class="mpro-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"<?php if ( in_array( $status, array( 'sent', 'error' ), true ) ) : ?> aria-describedby="<?php echo esc_attr( $notice_id ); ?>"<?php endif; ?>>
				<input type="hidden" name="action" value="mpro_submit">
				<input type="hidden" name="form_id" value="<?php echo esc_attr( (string) $form_id ); ?>">
				<input type="hidden" name="mpro_started_at" value="<?php echo esc_attr( (string) time() ); ?>">
				<?php wp_nonce_field( 'mpro_submit_' . $form_id, 'mpro_nonce' ); ?>
				...
				<?php if ( ! empty( $settings['honeypot'] ) ) : ?>
					<label class="mpro-honeypot" aria-hidden="true" for="<?php echo esc_attr( 'mpro-website-' . $form_id ); ?>">
						<?php esc_html_e( 'Leave this field empty', 'mpro-forms' ); ?>
						<input id="<?php echo esc_attr( 'mpro-website-' . $form_id ); ?>" name="website" tabindex="-1" autocomplete="off">
					</label>
				<?php endif; ?>
	...
	public static function handle_submission(): void {
		...
		$started = isset( $_POST['mpro_started_at'] ) ? absint( $_POST['mpro_started_at'] ) : 0;
		...
		if (
			! $form ||
			! isset( $_POST['mpro_nonce'] ) ||
			! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['mpro_nonce'] ) ), 'mpro_submit_' . $form_id ) ||
			! empty( $_POST['website'] ) ||
			$elapsed < 2 ||
		...
		if ( ! Entry_Repository::create( $form_id, $validation['data'], $context ) ) {
			$errors = array( '_form' => __( 'We could not save your submission. Please try again.', 'mpro-forms' ) );
	...
	private static function render_error_summary( array $fields, array $errors, string $fallback, string $notice_id, int $form_id ): void {
		...
		?>
		<div id="<?php echo esc_attr( $notice_id ); ?>" class="mpro-notice is-error mpro-error-summary" role="alert" tabindex="-1">
	...
	private static function render_field( array $field, string $select_label, string $yes_label, int $form_id, int $index, array $values, array $errors ): void {
		if ( 'section' === $field['type'] ) {
			echo '<section class="mpro-section"><h2>' . esc_html( $field['label'] ) . '</h2>';
			if ( $field['help'] ) {
		...
		$class        = $wide ? 'mpro-field mpro-field--wide' : 'mpro-field';
		$placeholder  = '' !== (string) ( $field['placeholder'] ?? '' )
			? ' placeholder="' . esc_attr( (string) $field['placeholder'] ) . '"'
			: '';

		if ( in_array( $field['type'], array( 'radio', 'scale' ), true ) ) {
			echo '<fieldset id="' . esc_attr( $field_id ) . '" class="' . esc_attr( $class . ' mpro-fieldset' . ( $error ? ' is-invalid' : '' ) ) . '"' . ( $described_by ? ' aria-describedby="' . esc_attr( $described_by ) . '"' : '' ) . ( $error ? ' aria-invalid="true" tabindex="-1"' : '' ) . '>';
			echo '<legend class="mpro-label">' . esc_html( self::label_text( $field ) ) . '</legend>';
			echo '<div class="mpro-choice-group' . ( 'scale' === $field['type'] ? ' mpro-scale' : '' ) . '">';
		...
		if ( 'checkbox' === $field['type'] ) {
			echo '<span class="mpro-label">' . esc_html( self::label_text( $field ) ) . '</span>';
			echo '<label class="mpro-checkbox" for="' . esc_attr( $field_id ) . '"><input id="' . esc_attr( $field_id ) . '" type="checkbox" name="' . esc_attr( $field['name'] ) . '" value="1"' . checked( $value, '1', false ) . ( $required ? ' required aria-required="true"' : '' ) . ( $described_by ? ' aria-describedby="' . esc_attr( $described_by ) . '"' : '' ) . ( $error ? ' aria-invalid="true"' : '' ) . '><span>' . esc_html( $yes_label ) . '</span></label>';
			self::render_help( $field, $help_id );
			self::render_error( $error, $error_id );
			echo '</div>';
			return;
		...
		echo '<label class="mpro-label" for="' . esc_attr( $field_id ) . '">' . esc_html( self::label_text( $field ) ) . '</label>';
	...
	private static function render_help( array $field, string $help_id ): void {
		if ( $field['help'] && $help_id ) {
			echo '<small id="' . esc_attr( $help_id ) . '" class="mpro-help">' . esc_html( $field['help'] ) . '</small>';
		}
	}

	private static function render_error( string $error, string $error_id ): void {
		if ( $error && $error_id ) {
			echo '<p id="' . esc_attr( $error_id ) . '" class="mpro-field-error">' . esc_html( $error ) . '</p>';
		}
	}

	private static function field_id( int $form_id, int $index, array $field ): string {
		return 'mpro-' . $form_id . '-' . $index . '-' . $field['name'];
	}
	...
	private static function redirect( string $url, string $status, int $form_id, string $token = '' ): void {
		...
		$url  = remove_query_arg( array( 'mpro_status', 'mpro_state', 'mpro_form' ), $url );
		$args = array(
			'mpro_status' => $status,
			'mpro_form'   => $form_id,
		);
		...
		if ( $token ) {
			$args['mpro_state'] = $token;
		}

		wp_safe_redirect( add_query_arg( $args, $url ) . '#mpro-form-' . $form_id );
		exit;
	}
}
