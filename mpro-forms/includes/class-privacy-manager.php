...
final class Privacy_Manager {
	private const EXPORTER_ID = 'mpro-forms-submissions';
	...
	public static function register_exporter( array $exporters ): array {
		$exporters[ self::EXPORTER_ID ] = array(
			'exporter_friendly_name' => __( 'MPRO Forms submissions', 'mpro-forms' ),
	...
	public static function register_eraser( array $erasers ): array {
		$erasers[ self::EXPORTER_ID ] = array(
			'eraser_friendly_name' => __( 'MPRO Forms submissions', 'mpro-forms' ),
	...
	public static function export_personal_data( string $email_address, int $page = 1 ): array {
		...
		foreach ( $entries as $entry ) {
			...
			$export_items[] = array(
				...
				'group_label' => __( 'Form submissions', 'mpro-forms' ),
				'item_id'     => 'mpro-entry-' . $entry['id'],
	...
	public static function erase_personal_data( string $email_address, int $page = 1 ): array {
		...
		foreach ( $entries as $entry ) {
			...
			$messages[]     = sprintf(
				...
				__( 'Personal values in submission %d could not be erased.', 'mpro-forms' ),
				(int) $entry['id']
			);
		}
	...
	private static function export_fields( array $entry ): array {
		...
		$items = array(
			array(
				'name'  => __( 'Form', 'mpro-forms' ),
				'value' => (string) ( $form['title'] ?? __( 'Deleted form', 'mpro-forms' ) ),
			),
			array(
				'name'  => __( 'Submitted on', 'mpro-forms' ),
