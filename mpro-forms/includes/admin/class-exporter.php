...
final class Exporter {
	private const ACTION = 'mpro_export';
	...
	public static function handle_export(): void {
		...
		if ( ! $form ) {
			wp_safe_redirect( Plugin::admin_url( Plugin::MENU_SLUG . '-tools', array( 'mpro_notice' => 'error' ) ) );
			exit;
		}
	...
	private static function stream_csv( int $form_id, array $columns ): void {
		...
		$header = array_merge(
			array( __( 'Entry ID', 'mpro-forms' ), __( 'Submitted on', 'mpro-forms' ), __( 'Status', 'mpro-forms' ) ),
			array_values( $columns ),
			array( __( 'Admin note', 'mpro-forms' ) )
		);
