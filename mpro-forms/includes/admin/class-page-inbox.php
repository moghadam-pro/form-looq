...
final class Page_Inbox {
	...
	private static function handle_single_action(): void {
		...
		$action   = isset( $_GET['mpro_action'] ) ? sanitize_key( wp_unslash( $_GET['mpro_action'] ) ) : '';
		...
		check_admin_referer( 'mpro_entry_' . $action . '_' . $entry_id );
	...
	private static function handle_bulk_action(): void {
		...
		check_admin_referer( 'bulk-mpro_entries' );
	...
	public static function render(): void {
		...
		<div class="wrap mpro-wrap">
			...
			Admin::header(
				__( 'Inbox', 'mpro-forms' ),
				__( 'Entries submitted through your forms, stored only on this site.', 'mpro-forms' )
			);
			Admin::render_notice();
			?>
			...
			<?php if ( ! $forms ) : ?>
				<div class="notice notice-info inline">
					<p>
						<?php esc_html_e( 'There are no forms yet, so there is nothing to show here.', 'mpro-forms' ); ?>
						<a href="<?php echo esc_url( Plugin::admin_url( Plugin::MENU_SLUG . '-new' ) ); ?>"><?php esc_html_e( 'Create your first form.', 'mpro-forms' ); ?></a>
					</p>
				</div>
				<?php return; ?>
			<?php endif; ?>

			<form method="get" class="mpro-inbox">
				<input type="hidden" name="page" value="<?php echo esc_attr( Plugin::MENU_SLUG . '-inbox' ); ?>">

				<div class="mpro-inbox__bar">
					<label for="mpro-inbox-form"><strong><?php esc_html_e( 'Form', 'mpro-forms' ); ?></strong></label>
					<select id="mpro-inbox-form" name="form" onchange="this.form.submit()">
					...
					<noscript><button type="submit" class="button"><?php esc_html_e( 'Show', 'mpro-forms' ); ?></button></noscript>

					<a class="button mpro-inbox__export" href="<?php echo esc_url( Plugin::admin_url( Plugin::MENU_SLUG . '-tools', array( 'form' => $form_id ) ) ); ?>">
						<?php esc_html_e( 'Export this form', 'mpro-forms' ); ?>
					</a>
				</div>
				...
				$table->prepare_items();
				$table->search_box( __( 'Search entries', 'mpro-forms' ), 'mpro-entry-search' );
				$table->display();
				?>
			</form>
		</div>
