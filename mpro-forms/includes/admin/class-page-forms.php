...
final class Page_Forms {
	...
	private static function handle_single_action(): void {
		...
		check_admin_referer( 'mpro_form_' . $action . '_' . $form_id );
	...
	private static function handle_bulk_action(): void {
		...
		check_admin_referer( 'bulk-mpro_forms' );
	...
	private static function redirect( string $notice ): void {
		wp_safe_redirect( Plugin::admin_url( Plugin::MENU_SLUG, array( 'mpro_notice' => $notice ) ) );
		exit;
	}
	...
	public static function render(): void {
		...
		<div class="wrap mpro-wrap">
			<?php Admin::header( __( 'Forms list', 'mpro-forms' ), __( 'Every form on this site, with its entries, views, and conversion rate.', 'mpro-forms' ) ); ?>

			<h2 class="screen-reader-text"><?php esc_html_e( 'Forms list', 'mpro-forms' ); ?></h2>

			<p class="mpro-page-actions">
				<a class="button button-primary" href="<?php echo esc_url( $new_url ); ?>">
					<?php esc_html_e( 'Add new form', 'mpro-forms' ); ?>
				</a>
			</p>
			...
			<form method="get">
				...
				<?php
				$table->search_box( __( 'Search forms', 'mpro-forms' ), 'mpro-form-search' );
				$table->display();
				?>
			</form>
		</div>
