...
final class Entries_List_Table extends \WP_List_Table {
	...
	public function get_columns(): array {
		...
		$columns['status']     = __( 'Status', 'mpro-forms' );
		$columns['created_at'] = __( 'Submitted', 'mpro-forms' );
	...
	protected function get_bulk_actions(): array {
		return array(
			'mark_read'   => __( 'Mark as read', 'mpro-forms' ),
			'mark_unread' => __( 'Mark as unread', 'mpro-forms' ),
			'delete'      => __( 'Delete', 'mpro-forms' ),
		);
	}

	public function no_items(): void {
		esc_html_e( 'No entries found.', 'mpro-forms' );
	}
	...
	protected function column_id( array $item ): string {
		...
		$actions = array(
			'view'   => sprintf( '<a href="%s">%s</a>', esc_url( $view_url ), esc_html__( 'View', 'mpro-forms' ) ),
			'delete' => sprintf(
				'<a class="submitdelete mpro-confirm" data-mpro-confirm="%s" href="%s">%s</a>',
				esc_attr__( 'Delete this entry permanently?', 'mpro-forms' ),
	...
	protected function extra_tablenav( $which ): void {
		...
		<div class="alignleft actions">
			<label class="screen-reader-text" for="mpro-entry-status"><?php esc_html_e( 'Filter by status', 'mpro-forms' ); ?></label>
			<select name="entry_status" id="mpro-entry-status">
				<option value=""><?php esc_html_e( 'All entries', 'mpro-forms' ); ?></option>
	...
	public function single_row( $item ): void {
		$classes = Entry_Repository::STATUS_UNREAD === $item['status'] ? 'mpro-entry-unread' : '';
