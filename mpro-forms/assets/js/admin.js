/**
 * MPRO Forms — shared admin behaviour.
 *
...
( function () {
	...
	var config = window.mproAdmin || {};

	function announce( message ) {
		var notice = document.createElement( 'div' );
		notice.className = 'notice notice-success is-dismissible mpro-flash';
		notice.setAttribute( 'role', 'status' );
		notice.innerHTML = '<p></p>';
		notice.querySelector( 'p' ).textContent = message;

		var anchor = document.querySelector( '.mpro-header' );
	...
	document.addEventListener( 'click', function ( event ) {
		var copyButton = event.target.closest( '[data-mpro-copy]' );
		if ( copyButton ) {
			event.preventDefault();
			copyFrom( copyButton.getAttribute( 'data-mpro-copy' ) );
			return;
		}

		var confirmLink = event.target.closest( '[data-mpro-confirm]' );
		if ( confirmLink && ! window.confirm( confirmLink.getAttribute( 'data-mpro-confirm' ) ) ) {
			event.preventDefault();
		}
	} );

	// Warn before navigating away from a builder form with unsaved edits.
	var builderForm = document.getElementById( 'mpro-builder-form' );
