...
.SYNOPSIS
  ...
  a `zip` binary. Produces dist/mpro-forms.zip with a single top-level
  `mpro-forms/` folder, forward-slash entry paths (required — a zip built
  with backslash paths does not extract as a folder on Linux hosting), and
  none of the development-only files that live outside the mpro-forms/
...
$PluginDir = Join-Path $RootDir 'mpro-forms'
$DistDir   = Join-Path $RootDir 'dist'
$ZipFile   = Join-Path $DistDir 'mpro-forms.zip'

if (-not (Test-Path $PluginDir)) {
    throw "Plugin directory not found: $PluginDir"
}

$HeaderLine = Select-String -Path (Join-Path $PluginDir 'mpro-forms.php') -Pattern '^\s*\*\s*Version:\s*(\S+)' | Select-Object -First 1
if (-not $HeaderLine) { throw 'Could not read Version header from mpro-forms.php.' }
...
# Verify: every entry lives under mpro-forms/, uses forward slashes, and no
...
try {
    ...
    foreach ($entry in $verify.Entries) {
        $name = $entry.FullName
        if (-not $name.StartsWith('mpro-forms/') -or $name.Contains('\') -or $name.Contains('__MACOSX') -or (Split-Path $name -Leaf).StartsWith('._')) {
            $badPaths += $name
        }
    }
