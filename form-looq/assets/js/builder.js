/**
 * Form LOOQ — drag-and-drop form builder.
 *
...
( function () {
	...
	var config = window.looqBuilder || {};
	var LABELS = config.labels || {};
	var TYPES = config.types || [];
	var MAX_FIELDS = 50;

	var canvas = document.getElementById( 'looq-canvas' );
	var palette = document.getElementById( 'looq-palette' );
	var input = document.getElementById( 'looq-fields-input' );
	var form = document.getElementById( 'looq-builder-form' );
	...
	function labelled( labelText, control, id ) {
		var wrap = el( 'p', 'looq-card__control' );
	...
	function renderCard( field, index ) {
		...
		var card = el( 'div', 'looq-card' );
		card.setAttribute( 'draggable', 'true' );
		card.setAttribute( 'data-index', String( index ) );

		var head = el( 'div', 'looq-card__head' );
		var handle = el( 'span', 'looq-card__handle dashicons dashicons-move' );
		handle.setAttribute( 'aria-hidden', 'true' );
		head.appendChild( handle );

		var icon = el( 'span', 'looq-card__icon dashicons dashicons-' + meta.icon );
		icon.setAttribute( 'aria-hidden', 'true' );
		head.appendChild( icon );

		head.appendChild( el( 'strong', 'looq-card__type', meta.label ) );
		head.appendChild( el( 'span', 'looq-card__summary', field.label || '' ) );

		var tools = el( 'span', 'looq-card__tools' );
		...
		var body = el( 'div', 'looq-card__body' );
		var prefix = 'looq-field-' + index + '-';

		body.appendChild( labelled( LABELS.label, textInput( field.label, function ( value ) {
			field.label = value;
			var summary = card.querySelector( '.looq-card__summary' );
			if ( summary ) {
		...
		if ( 'section' !== field.type ) {
			var flags = el( 'p', 'looq-card__flags' );
			...
			var widthLabel = el( 'label', 'looq-card__width' );
		...
		card.addEventListener( 'drop', function ( event ) {
			...
			var newType = event.dataTransfer.getData( 'looq/type' );
			if ( newType ) {
	...
	function toolButton( icon, label, onClick ) {
		...
		button.type = 'button';
		button.className = 'looq-card__tool';
		button.title = label;
	...
	function render() {
		...
		if ( ! fields.length ) {
			canvas.appendChild( el( 'p', 'looq-canvas__empty', LABELS.empty ) );
			sync();
			return;
		}
	...
	if ( palette ) {
		palette.addEventListener( 'click', function ( event ) {
			var button = event.target.closest( '.looq-palette__item' );
			if ( button ) {
				insert( makeField( button.getAttribute( 'data-looq-type' ) ) );
			}
		} );
		...
		palette.addEventListener( 'dragstart', function ( event ) {
			...
			event.dataTransfer.setData( 'looq/type', button.getAttribute( 'data-looq-type' ) );
			event.dataTransfer.setData( 'text/plain', button.getAttribute( 'data-looq-type' ) );
		} );
	}
