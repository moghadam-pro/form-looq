...
final class Page_Forms {
	...
	private static function handle_single_action(): void {
		...
		check_admin_referer( 'looq_form_' . $action . '_' . $form_id );
	...
	private static function handle_bulk_action(): void {
		...
		check_admin_referer( 'bulk-looq_forms' );
	...
	private static function redirect( string $notice ): void {
		wp_safe_redirect( Plugin::admin_url( Plugin::MENU_SLUG, array( 'looq_notice' => $notice ) ) );
		exit;
	}
	...
	public static function render(): void {
		...
		<div class="wrap looq-wrap">
			<?php Admin::header( __( 'Forms list', 'form-looq' ), __( 'Every form on this site, with its entries, views, and conversion rate.', 'form-looq' ) ); ?>

			<h2 class="screen-reader-text"><?php esc_html_e( 'Forms list', 'form-looq' ); ?></h2>

			<p class="looq-page-actions">
				<a class="button button-primary" href="<?php echo esc_url( $new_url ); ?>">
					<?php esc_html_e( 'Add new form', 'form-looq' ); ?>
				</a>
			</p>
			...
			<form method="get">
				...
				<?php
				$table->search_box( __( 'Search forms', 'form-looq' ), 'looq-form-search' );
				$table->display();
				?>
			</form>
		</div>
