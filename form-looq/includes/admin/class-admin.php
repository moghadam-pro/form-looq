...
final class Admin {
	...
	public static function init(): void {
		...
		add_filter( 'plugin_action_links_' . plugin_basename( FORM_LOOQ_FILE ), array( self::class, 'action_links' ) );
	...
	public static function body_class( string $classes ): string {
		return self::is_plugin_screen() ? $classes . ' looq-admin ' : $classes;
	}
	...
	public static function enqueue( string $hook ): void {
		...
		wp_enqueue_script(
			'looq-admin',
			plugins_url( 'assets/js/admin.js', FORM_LOOQ_FILE ),
			array(),
			FORM_LOOQ_VERSION,
			true
		);
		...
		wp_localize_script(
			...
			'looqAdmin',
			array(
				'copied'       => __( 'Copied to clipboard.', 'form-looq' ),
				'copyFailed'   => __( 'Copy failed. Select the text and copy it manually.', 'form-looq' ),
				'confirmLeave' => (bool) Settings::get( 'confirm_before_leaving', true ),
				'leaveWarning' => __( 'You have unsaved changes.', 'form-looq' ),
			)
		);
	}
	...
	public static function no_conflict(): void {
		...
		$admin_url  = admin_url();
		$plugin_url = plugins_url( '', FORM_LOOQ_FILE );
	...
	public static function header( string $title, string $subtitle = '' ): void {
		?>
		<div class="looq-header">
			<img class="looq-header__logo" src="<?php echo esc_url( Plugin::logo_url() ); ?>" alt="" width="32" height="32">
			<div class="looq-header__identity">
				<span class="looq-header__name"><?php echo esc_html( Plugin::name() ); ?></span>
				<span class="looq-header__version">v<?php echo esc_html( Plugin::version() ); ?></span>
			</div>
			<div class="looq-header__page">
				<h1 class="looq-header__title"><?php echo esc_html( $title ); ?></h1>
				<?php if ( '' !== $subtitle ) : ?>
					<p class="looq-header__subtitle"><?php echo esc_html( $subtitle ); ?></p>
				<?php endif; ?>
			</div>
		</div>
		<?php
	...
	public static function action_links( array $links ): array {
		array_unshift(
			$links,
			'<a href="' . esc_url( Plugin::admin_url() ) . '">' . esc_html__( 'Forms', 'form-looq' ) . '</a>',
			'<a href="' . esc_url( Plugin::admin_url( Plugin::MENU_SLUG . '-settings' ) ) . '">' . esc_html__( 'Settings', 'form-looq' ) . '</a>'
		);
	...
	public static function guard(): void {
		if ( ! Plugin::current_user_can() ) {
			wp_die( esc_html__( 'You do not have permission to manage forms.', 'form-looq' ), 403 );
		}
	}
	...
	public static function render_notice(): void {
		...
		$message = isset( $_GET['looq_notice'] ) ? sanitize_key( wp_unslash( $_GET['looq_notice'] ) ) : '';
		...
		$notices = array(
			'created'     => array( 'success', __( 'Form created.', 'form-looq' ) ),
			'updated'     => array( 'success', __( 'Form saved.', 'form-looq' ) ),
			'duplicated'  => array( 'success', __( 'Form duplicated.', 'form-looq' ) ),
			'deleted'     => array( 'success', __( 'Form deleted.', 'form-looq' ) ),
			'settings'    => array( 'success', __( 'Settings saved.', 'form-looq' ) ),
			'entry-note'  => array( 'success', __( 'Note saved.', 'form-looq' ) ),
			'entries'     => array( 'success', __( 'Entries updated.', 'form-looq' ) ),
			'error'       => array( 'error', __( 'The request could not be completed.', 'form-looq' ) ),
		);
