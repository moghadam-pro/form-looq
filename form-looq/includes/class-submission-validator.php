...
final class Submission_Validator {
	...
	public static function validate( array $fields, array $request ): array {
		...
		$allowed_keys = array_merge(
			$field_keys,
			array( 'action', 'form_id', 'looq_nonce', 'looq_started_at', 'website', '_wp_http_referer' )
		);
		$unexpected   = array_diff( array_keys( $request ), $allowed_keys );

		if ( $unexpected ) {
			$errors['_form'] = __( 'The submission contained unexpected fields. Please reload the page and try again.', 'form-looq' );
		}
		...
		switch ( $code ) {
			case 'required':
				return sprintf(
					...
					__( '%s is required.', 'form-looq' ),
					$field['label']
				);
			case 'email':
				return __( 'Enter a valid email address.', 'form-looq' );
			case 'number':
				return __( 'Enter a valid number.', 'form-looq' );
			case 'tel':
				return __( 'Enter a valid telephone number.', 'form-looq' );
			case 'choice':
				return __( 'Select one of the available options.', 'form-looq' );
			case 'length':
				return __( 'This value is longer than the allowed limit.', 'form-looq' );
			default:
				return __( 'Review this field and try again.', 'form-looq' );
		}
	}
