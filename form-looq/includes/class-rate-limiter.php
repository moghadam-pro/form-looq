...
final class Rate_Limiter {
	...
	public static function init(): void {
		add_action( 'admin_post_looq_submit', array( self::class, 'guard_submission' ), 5 );
		add_action( 'admin_post_nopriv_looq_submit', array( self::class, 'guard_submission' ), 5 );
	}
	...
	public static function guard_submission(): void {
		...
		if ( ! apply_filters( 'form_looq_rate_limit_enabled', true, $form_id ) ) {
			return;
		}
		...
		$errors = array(
			'_form' => __( 'Too many submission attempts. Please wait a little and try again.', 'form-looq' ),
		);
	...
	private static function client_identity(): string {
		...
		return (string) apply_filters( 'form_looq_rate_limit_identity', $address );
	}

	private static function transient_key( int $form_id, string $window, string $hash ): string {
		return 'form_looq_rate_' . $form_id . '_' . $window . '_' . substr( $hash, 0, 32 );
	}
	...
	private static function redirect( int $form_id, string $token ): void {
		...
		wp_safe_redirect( $url . '#looq-form-' . $form_id );
		exit;
	}
}
