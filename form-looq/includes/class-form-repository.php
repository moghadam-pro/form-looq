...
final class Form_Repository {
	...
	private const CACHE_GROUP = 'looq_forms';
	...
	public static function statuses(): array {
		return array(
			self::STATUS_ACTIVE   => __( 'Active', 'form-looq' ),
			self::STATUS_INACTIVE => __( 'Inactive', 'form-looq' ),
			self::STATUS_DRAFT    => __( 'Draft', 'form-looq' ),
		);
	}
	...
	public static function create( array $data ): int {
		 ...
		 */
		do_action( 'form_looq_form_created', $form_id );
	...
	public static function update( int $form_id, array $data ): bool {
		 ...
		 */
		do_action( 'form_looq_form_updated', $form_id );
	...
	public static function duplicate( int $form_id ): int {
		...
		return self::create(
			array(
				...
				'title'       => sprintf( __( '%s (copy)', 'form-looq' ), $form['title'] ),
	...
	public static function delete( int $form_id ): bool {
		 ...
		 */
		do_action( 'form_looq_form_deleted', $form_id );
	...
	public static function record_view( int $form_id ): void {
		...
		if ( $form_id <= 0 || ! apply_filters( 'form_looq_track_views', true, $form_id ) ) {
			return;
		}
	...
	private static function sanitize_title( string $title ): string {
		...
		$title = '' !== $title ? $title : __( 'Untitled form', 'form-looq' );
	...
	public static function sanitize_settings( array $settings ): array {
		...
		if ( '' === $clean['submit_label'] ) {
			$clean['submit_label'] = __( 'Submit', 'form-looq' );
		}

		if ( '' === $clean['success_message'] ) {
			$clean['success_message'] = __( 'Thank you. Your submission has been received.', 'form-looq' );
		}
